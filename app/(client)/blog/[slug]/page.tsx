            ...
            <div className="text-xs flex items-center gap-5 my-7">
              <div className="flex items-center relative group cursor-pointer">
                {blog?.blogcategories?.map(
                  (item: { title: string | null } | null, index: number) => (
                    <p
                      key={index}
              ...
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-shop_dark_green hoverEffect">
                <Calendar size={15} />{" "}
                {blog?.publishedAt ? dayjs(blog.publishedAt).format("MMMM D, YYYY") : 'Unknown Date'}
                <span className="absolute left-0 -bottom-1.5 bg-lightColor/30 inline-block w-full h-[2px] group-hover:bg-shop_dark_green hoverEffect" />
              </p>
            </div>
            <h2 className="text-2xl font-bold my-5">{blog?.title || 'Untitled Blog'}</h2>
      ...
      <div className="border border-lightColor p-5 rounded-md">
        ...
        <div className="space-y-2 mt-2">
          {categories?.map(({ blogcategories }, index) => (
            ...
            >
              <p>{blogcategories && blogcategories.length > 0 ? blogcategories[0]?.title || 'Uncategorized' : 'Uncategorized'}</p>
